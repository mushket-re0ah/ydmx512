...
class TextInputPainterManager:
    BORDER_PADDING = 1

    def __init__(self, text_input):
        self.text_input = text_input

        self.background_rect_color: Optional[Color] = Color(text_input.background_color)
        self.background_rect: Optional[Rectangle] = Rectangle(pos=text_input.pos, size=text_input.size)
        self.border_color: Optional[Color] = None
        self.border_line: Optional[Line] = None
        self.cursor_rect_color: Optional[Color] = None
        self.cursor_rect: Optional[Rectangle] = None
        self.foreground_color: Optional[Color] = None
        self.line_list: List[Rectangle] = []
        self.selection_color: Optional[Color] = None
        self.selection_rect_list: List[Rectangle] = []

        Clock.schedule_once(self._setup_triggers, 0)

    def _setup_triggers(self, _):
        self.update_canvas_order_ev = Clock.create_trigger(self.update_canvas_order, 1/24)

        update_lines_ev = Clock.create_trigger(self._update_lines, -1)
        update_background_ev = Clock.create_trigger(self._update_background, -1)
        update_cursor_ev = Clock.create_trigger(self._update_cursor, -1)
        update_border_ev = Clock.create_trigger(self._update_border, -1)
        update_selection_ev = Clock.create_trigger(self._update_selection, -1)
        self.update_lines_ev = update_lines_ev
        self.update_background_ev = update_background_ev
        self.update_cursor_ev = update_cursor_ev
        self.update_border_ev = update_border_ev
        self.update_selection_ev = update_selection_ev
        text_input = self.text_input

        text_input.bind(
            pos=update_lines_ev,
            size=update_lines_ev,
            halign=update_lines_ev,
            foreground_color=update_lines_ev,
            text=update_lines_ev,
            _lines=update_lines_ev,
            scroll_y=update_lines_ev,
            scroll_x=update_lines_ev
        )
        text_input.bind(
            pos=update_background_ev,
            size=update_background_ev,
            background_color=update_background_ev
        )
        text_input.bind(
            focus=update_cursor_ev,
            _cursor_blink=update_cursor_ev,
            pos=update_cursor_ev,
            size=update_cursor_ev,
            padding=update_cursor_ev,
            cursor_color=update_cursor_ev,
            _cursor_visual_pos=update_cursor_ev,
            _cursor_visual_height=update_cursor_ev,
            cursor_width=update_cursor_ev,
        )
        text_input.bind(
            pos=update_border_ev,
            size=update_border_ev,
            border_color=update_border_ev
        )
        text_input.bind(
            _selection_from=update_selection_ev,
            _selection_to=update_selection_ev
        )
        update_lines_ev()
        update_background_ev()
        update_cursor_ev()
        update_border_ev()
        update_selection_ev()
    ...
    def update_canvas_order(self, _):
        ...
        if self.selection_color and self.selection_rect_list:
            ...
            for rect in self.selection_rect_list:
                canvas.add(rect)

        if self.foreground_color and self.line_list:
            canvas.add(self.foreground_color)
        ...
        if self.border_color and self.border_line:
            ...
            canvas.add(self.border_line)

    def _update_background(self, *args):
        self.background_rect_color.rgba = self.text_input.background_color
        self.background_rect.pos = self.text_input.pos
        self.background_rect.size = self.text_input.size
        self.update_canvas_order_ev()

    def _update_border(self, *args):
        if not self._is_border_visible():
            if self.border_line:
        ...
        rect_coords = self._get_border_rectangle()
        color = self.text_input.border_color

        if not self.border_line:
            self.border_color = Color(*color)
            self.border_line = Line(rectangle=rect_coords, width=1)
            self.update_canvas_order_ev()
        else:
            self.border_color.rgba = color
            self.border_line.rectangle = rect_coords

    def _is_border_visible(self) -> bool:
        return self.text_input.border_color[0] > 0.001
    ...
    def _is_cursor_visible(self) -> bool:
        ti = self.text_input
        return (
            ti.focus and not ti._cursor_blink and
            ti.x + ti.padding[0] <= ti._cursor_visual_pos[0] <=
            ti.x + ti.width - ti.padding[2]
        )

    def _update_cursor(self, *args):
        if not self._is_cursor_visible():
            if self.cursor_rect:
                self.cursor_rect_color = None
        ...
        if not self.cursor_rect:
            self.cursor_rect_color = Color(*color)
            self.cursor_rect = Rectangle(pos=pos, size=size)
            self.update_canvas_order_ev()
        else:
            self.cursor_rect_color.rgba = color
            self.cursor_rect.pos = pos
            self.cursor_rect.size = size

    def _update_lines(self, *args):
        text_input = self.text_input
        lines = text_input._lines
        ...
        if not self.foreground_color:
            self.foreground_color = Color(*text_input.foreground_color)
        else:
        ...
        base_dir = text_input.base_direction
        auto_halign_r = halign == 'auto' and base_dir and 'rtl' in base_dir
        
        self.line_list.clear()
        labels = text_input._lines_labels
        rects = text_input._lines_rects
        
        for i, line_text in enumerate(lines):
            line_y_pos = y - i * dy
            if min_y <= line_y_pos <= max_y + dy:
                self._draw_line(
                    y=line_y_pos,
                    texture=labels[i],
                    rect=rects[i],
                    auto_halign_r=auto_halign_r
                )

        self.update_canvas_order_ev()

    def _draw_line(self, y: float, texture, rect: Rectangle, auto_halign_r: bool):
        text_input = self.text_input
        padding_left, padding_top, padding_right, padding_bottom = text_input.padding

        viewport_width = text_input.width - padding_left - padding_right
        ...
        self.line_list.append(rect)

    def _create_line_label(self, text):
        text_input = self.text_input

        ntext = text.replace(u'\n', u'').replace(u'\t', u' ' * text_input.tab_width)

        kw = text_input._get_line_options()
        cid = f"{ntext}\0{str(kw)}"
        texture = Cache_get('text_input.label', cid)

        if texture is None:
            if not ntext:
                texture = Texture.create(size=(1, 1))
                Cache_append('text_input.label', cid, texture)
                return texture
    ...
    def _update_selection(self, *args):
        ...
        if not text_input._selection:
            if self.selection_color:
                self.selection_color = None
                self.selection_rect_list.clear()
                self.update_canvas_order_ev()
            return

        if not self.selection_color:
            self.selection_color = Color(*text_input.selection_color)
        else:
            self.selection_color.rgba = text_input.selection_color

        self.selection_rect_list.clear()

        a, b = sorted((text_input.selection_from, text_input.selection_to))
        selection_start = text_input.get_cursor_from_index(a)
        selection_end = text_input.get_cursor_from_index(b)

        label_cached = text_input._label_cached
        rects = text_input._lines_rects

        for i, rect in enumerate(rects):
             if selection_start[1] <= i <= selection_end[1]:
                 self._draw_selection(rect, i, selection_start, selection_end, label_cached)

        self.update_canvas_order_ev()

    def _draw_selection(
            self, line_rect: Rectangle, line_num: int,
        ...
        if line_num == selection_start_row:
            beg += text_input._get_text_width(
                lines[line_num][:selection_start_col],
                text_input.tab_width,
                label_cached
            )

        if line_num == selection_end_row:
            end = x + text_input._get_text_width(
                lines[line_num][:selection_end_col],
                text_input.tab_width,
                label_cached
            )
        ...
        self.selection_rect_list.append(
            Rectangle(pos=(beg, y), size=(end - beg, h))
        )


class CentralizedHotkeyTextInput(KeyboardBehavior, AnimationBehavior, TooltipBehavior, FocusBehavior, Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.interesting_keys = {
            ...
            SDL_SCANCODE_HOME: 'cursor_home',
            SDL_SCANCODE_END: 'cursor_end',
            SDL_SCANCODE_PAGEUP: 'cursor_pgup',
            SDL_SCANCODE_PAGEDOWN: 'cursor_pgdown',
            SDL_SCANCODE_LSHIFT: 'shift_L',
            SDL_SCANCODE_RSHIFT: 'shift_R',
            SDL_SCANCODE_LCTRL: 'ctrl_L',
            SDL_SCANCODE_RCTRL: 'ctrl_R',
            SDL_SCANCODE_LALT: 'alt_L',
            SDL_SCANCODE_RALT: 'alt_R'
        }
        ...
        _textinput_list.append(ref(self, CentralizedHotkeyTextInput._reload_remove_observer))

        if platform == 'linux':
            self._ensure_clipboard()
        self.painter_manager = TextInputPainterManager(self)
    ...
    def on_mouse_move(self, _):
        if self.hover and not self.disabled:
            cursor_manager.set_cursor("ibeam")
    ...
    #         frozenset({"up"}): lambda: self._on_press_cursor_action("cursor_up"),
    #         frozenset({"down"}): lambda: self._on_press_cursor_action("cursor_down"),
    #         frozenset({"left"}): lambda: self._on_press_cursor_action("cursor_left"),
    #         frozenset({"right"}): lambda: self._on_press_cursor_action("cursor_right"),
    #         frozenset({"home"}): lambda: self._on_press_cursor_action("cursor_home"),
    #         frozenset({"end"}): lambda: self._on_press_cursor_action("cursor_end"),
    #         frozenset({"pageup"}): lambda: self._on_press_cursor_action("cursor_pgup"),
    #         frozenset({"pagedown"}): lambda: self._on_press_cursor_action("cursor_pgdown"),

    #         frozenset({"ctrl", "up"}): lambda: self._on_press_cursor_action("cursor_up"),
    #         frozenset({"ctrl", "down"}): lambda: self._on_press_cursor_action("cursor_down"),
    #         frozenset({"ctrl", "left"}): lambda: self._on_press_cursor_action("cursor_left"),
    #         frozenset({"ctrl", "right"}): lambda: self._on_press_cursor_action("cursor_right"),
    #         frozenset({"ctrl", "home"}): lambda: self._on_press_cursor_action("cursor_home"),
    #         frozenset({"ctrl", "end"}): lambda: self._on_press_cursor_action("cursor_end"),
    #         frozenset({"ctrl", "pageup"}): lambda: self._on_press_cursor_action("cursor_pgup"),
    #         frozenset({"ctrl", "pagedown"}): lambda: self._on_press_cursor_action("cursor_pgdown"),

    #         frozenset({"shift", "up"}): lambda: self._on_press_cursor_action("cursor_up"),
    #         frozenset({"shift", "down"}): lambda: self._on_press_cursor_action("cursor_down"),
    #         frozenset({"shift", "left"}): lambda: self._on_press_cursor_action("cursor_left"),
    #         frozenset({"shift", "right"}): lambda: self._on_press_cursor_action("cursor_right"),
    #         frozenset({"shift", "home"}): lambda: self._on_press_cursor_action("cursor_home"),
    #         frozenset({"shift", "end"}): lambda: self._on_press_cursor_action("cursor_end"),
    #         frozenset({"shift", "pageup"}): lambda: self._on_press_cursor_action("cursor_pgup"),
    #         frozenset({"shift", "pagedown"}): lambda: self._on_press_cursor_action("cursor_pgdown"),

    #         frozenset({"ctrl", "shift", "up"}): lambda: self._on_press_cursor_action("cursor_up"),
    #         frozenset({"ctrl", "shift", "down"}): lambda: self._on_press_cursor_action("cursor_down"),
    #         frozenset({"ctrl", "shift", "left"}): lambda: self._on_press_cursor_action("cursor_left"),
    #         frozenset({"ctrl", "shift", "right"}): lambda: self._on_press_cursor_action("cursor_right"),
    #         frozenset({"ctrl", "shift", "home"}): lambda: self._on_press_cursor_action("cursor_home"),
    #         frozenset({"ctrl", "shift", "end"}): lambda: self._on_press_cursor_action("cursor_end"),
    #         frozenset({"ctrl", "shift", "pageup"}): lambda: self._on_press_cursor_action("cursor_pgup"),
    #         frozenset({"ctrl", "shift", "pagedown"}): lambda: self._on_press_cursor_action("cursor_pgdown"),

    #         frozenset({"ctrl", "alt", "up"}): lambda: self._on_press_cursor_action("cursor_up"),
    #         frozenset({"ctrl", "alt", "down"}): lambda: self._on_press_cursor_action("cursor_down"),
    #         frozenset({"ctrl", "alt", "left"}): lambda: self._on_press_cursor_action("cursor_left"),
    #         frozenset({"ctrl", "alt", "right"}): lambda: self._on_press_cursor_action("cursor_right"),
    #         frozenset({"ctrl", "alt", "home"}): lambda: self._on_press_cursor_action("cursor_home"),
    #         frozenset({"ctrl", "alt", "end"}): lambda: self._on_press_cursor_action("cursor_end"),
    #         frozenset({"ctrl", "alt", "pageup"}): lambda: self._on_press_cursor_action("cursor_pgup"),
    #         frozenset({"ctrl", "alt", "pagedown"}): lambda: self._on_press_cursor_action("cursor_pgdown"),

    #         frozenset({"delete"}): self._on_press_delete,

    #         frozenset({"ctrl", "a"}): self.select_all,
    #         frozenset({"ctrl", "c"}): self.copy,
    #         frozenset({"ctrl", "v"}): self.paste,
    #         frozenset({"ctrl", "x"}): lambda: self._cut(self.selection_text),
    #         frozenset({"ctrl", "z"}): self.do_undo,
    #         frozenset({"ctrl", "shift", "z"}): self.do_redo,
    #         frozenset({"enter"}): self._on_press_enter,
    #         frozenset({"backspace"}): self._on_press_backspace,
    #     }

    # def create_hotkeys_up(self) -> dict:
    #     return {
    #         frozenset({"shift"}): self._on_up_shift,
    #         frozenset({"ctrl"}): self._on_up_ctrl,
    #         frozenset({"alt"}): self._on_up_alt,
    #     }

    # def _on_press_cursor_action(self, action: str):
    #     cc, cr = self.cursor
    ...
    #     if self._selection and self.multiline:
    #         self.delete_selection()
    #     else:
    #         cursor = self.cursor
    ...
    #     if scancode_is_shift(scancode): 
    #         self._on_press_shift()
    #     elif scancode_is_ctrl(scancode):
    #         self._on_press_ctrl()
    #     elif scancode_is_alt(scancode):
    #         self._on_press_alt()
    #     else:
    #         print("insert what", keycode)
    #         self.insert_text(keycode)

    # def _on_press_shift(self):
    #     if not self._selection:
    #         self._selection_from = self._selection_to = self.cursor_index()
    #         self._selection = True
    #     self._selection_finished = False

    # def _on_press_ctrl(self):
    #     self._ctrl_l = True
    #     self._ctrl_r = True

    # def _on_press_alt(self):
    #     self._alt_l = True
    #     self._alt_r = True

    # def on_key_up(self, scancode: int, keycode: str):
    #     print("on_key_up", self, scancode, keycode)
    #     if scancode_is_shift(scancode): 
    #         self._on_up_shift()
    #     elif scancode_is_ctrl(scancode):
    #         self._on_up_ctrl()
    ...
    # def _on_up_shift(self):
    #     if self._selection:
    #         self._update_selection(True)

    # def _on_up_ctrl(self):
    #     self._ctrl_l = False
    #     self._ctrl_r = False
    ...
    def on_key_down(self, scancode: int, keycode: str):
        ...
        is_interesting_key = scancode in self.interesting_keys.keys()

        if self._editable and text and not is_interesting_key:
            if win.managed_textinput:
                return
            if self._selection:
                self.delete_selection()
        ...
        if scancode == SDL_SCANCODE_ESCAPE:  # escape
            self.focus = False
            return True
        elif scancode == SDL_SCANCODE_TAB:  # tab
            self.delete_selection()
            self.insert_text(u'\t')
            return True

        k = self.interesting_keys.get(scancode)
        if k:
            key = (None, None, k, 1)
            self._key_down(key)
    ...
    def keyboard_on_key_down(self, window, keycode, text, modifiers):
        ...
        if text and is_shortcut and not is_interesting_key:
            ...
            print(first_char, self._command)
    ...
    _selection_from = NumericProperty(None, allownone=True)
    _selection_to = NumericProperty(None, allownone=True)
