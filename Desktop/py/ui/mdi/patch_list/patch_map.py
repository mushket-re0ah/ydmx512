...
class PatchMap(AutoUnbindBehavior, SectionPanel):
    patch_list = ObjectProperty()
    workspace_manager = ObjectProperty()

    selected = AliasProperty(lambda self: self.workspace_manager.workspace_now.box.selected)
    ...
    def on_workspace_opened(self, _, workspace_index: int, workspace: WorkspaceBehavior):
        self.unbind_from(self.workspace_now.box)
        self.workspace_now = workspace
        self.bind_to(workspace.box, selected=self._dispatch_selected)
